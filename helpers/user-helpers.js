...
module.exports = {
    ...
    //     return new Promise((resolve, reject) => {
    //         try{
    //         const wishlist = db.get().collection(collection.WISHLIST_COLLECTION).find().toArray()
    //         resolve(wishlist)
    //     } catch (err) {
    //         reject(err)
    //     }
    ...
    //     return new Promise((resolve, reject) => {
    //         try{
    //             const products=db.get().collection(collection.PRODUCT_COLLECTION).findOne({_id:objectId(proId)})
    //             const {_id, name, actual_price, description}=products
    //             const wishlistObj={
    ...
    changeProductQuantity: (details) => {
        return new Promise(async (resolve, reject) => {
            try {
                let { cartId, proId, count, quantity, subtotal } = details
